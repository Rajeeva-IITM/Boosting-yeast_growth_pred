...
def get_shap_values(model_path: Path, data_path: Path, fold: int) -> pl.DataFrame:
    ...
    shap_mean = (
        shap_df.apply(lambda x: np.mean(np.abs(x)))
        .sort_values(ascending=False)
        .reset_index()
    )
    # shap_mean = shap_mean[shap_mean > 0].reset_index()
    shap_mean = (
        pl.from_pandas(shap_mean)
        .with_columns(pl.lit(fold).alias("Fold"))
        .rename({"index": "Feature", "0": "Value"})
    )
    return shap_mean


def get_shap_folds(model_paths: Path, data_path: Path) -> pl.DataFrame:
    """Compute SHAP values for multiple models and concatenate them.
    ...
    for fold, model_path in enumerate(model_paths):
        df_fold = get_shap_values(model_path, data_path, fold)
